...
หากต้องการล้างรายการ session และความสามารถ Reply ที่ Bridge จำไว้ทั้งหมด ใช้:
...
- `/sessions` จะไม่มีรายการ
- ข้อความทั่วไปจะยังสั่ง Codex ไม่ได้
- Reply ข้อความ completion เก่าจะไม่พบ session
- ต้องรอให้ Codex ทำงานรอบใหม่และ Stop hook ส่ง completion ใหม่เข้ามาก่อน
- Codex task, transcript และประวัติ `/jobs` จริงไม่ถูกลบ

หากยังมีงานกำลังรันหรือรอคิว Bridge จะปฏิเสธ `/clearall` ให้รอจนงานจบก่อน คำสั่งนี้มี alias เป็น `/clear-all`
...
กรณีสร้างหรือตั้ง Telegram chatbot ใหม่ด้วย setup เวอร์ชันเก่า runtime `.env` อาจถูกเขียนใหม่และเปลี่ยน `CODEX_BIN` กลับเป็น `codex` แม้ `.env` ในโปรเจกต์มี absolute path ที่ถูกต้อง อาการในหน้าต่าง Bridge คือ `spawn codex ENOENT` เวอร์ชันปัจจุบันจะรักษา executable ที่ใช้ได้จาก runtime config เดิมหรือ `.env` ของโปรเจกต์ให้อัตโนมัติ
